...
ID <- read.xlsx("/Users/Madshove/OneDrive/Mads/golem/lister/ensyvni.xlsx") %>%
  filter(MF == "Nej", Navn != "Inger Støjberg", Navn != "Franciska Rosenkilde",
...
# Loop through all parties and leaders
for(i in ID$page_id) {
  # Printing info to console
  cat("Downloading data for user", i, "...\n")
  # Scrape
  L[[i]] <- get_targeting(i, timeframe = "LAST_30_DAYS")
}

D <- do.call(plyr::rbind.fill, L)

D <- D %>%
  rename(page_id = internal_id)

D <- merge(x = D, y = ID[ , c("page_id", "FB_Navn", "Parti")], by = "page_id")

D$total_spend_pct <- sprintf("%.5f", D$total_spend_pct)
...
# save RDS
saveRDS(D, "/Users/Madshove/OneDrive/Mads/golem/ensyvni/fb_ads/Parti_detailed_targeting.rds")
saveRDS(D, paste("/Users/Madshove/OneDrive/Mads/golem/ensyvni/fb_ads/Parti_detailed_targeting_", format(Sys.Date()-1, "%Y-%m-%d"), ".rds", sep = ""))
